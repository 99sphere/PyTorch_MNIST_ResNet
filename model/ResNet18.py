...
class ResNet18(nn.Module):
    def __init__(self, in_channels, out_channels, nker=64, nblk=[2,2,2,2]):
        ...
        for i, n in enumerate(nblk):
            ...
            for j in range(n):
                if i != 0 and j == 0:
                  res_blocks.append(ResBlock(in_channels, out_channels, kernel_size=3, stride=2, padding=1))                
                else:
                  res_blocks.append(ResBlock(in_channels, out_channels, kernel_size=3, stride=1, padding=1))                
