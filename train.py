...
if __name__ == "__main__":
    ...
    mnist_train_dataloader = DataLoader(mnist_train_dataset, batch_size=BATCH_SIZE, shuffle=True, num_workers=1)
    mnist_test_dataloader = DataLoader(mnist_test_dataset, batch_size=BATCH_SIZE, shuffle=True, num_workers=1)
    ...
    with open(os.path.join(log_dir, str(model)+'_3EPOCH_train_log.csv'), 'w') as log:
        model.train()
        for epoch in range(EPOCH):
            for iter, (img, label) in enumerate(mnist_train_dataloader):
                ...
                print("Iter [%3d/%3d] | Train Loss %.4f | Train Acc %.4f | Valid Loss %.4f | Valid Acc %.4f" %
                    (epoch * len(mnist_train_dataloader) + iter, EPOCH * len(mnist_train_dataloader), train_loss, train_acc, valid_loss, valid_acc))
        
                log.write('%d,%.4f,%.4f,%.4f,%.4f\n'%(epoch * len(mnist_train_dataloader) + iter, train_loss, train_acc, valid_loss, valid_acc))